...
class AppDetailsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (_) =>
          locator<MarketplaceCubit>()..loadAppDetails(int.parse(appId)),
          ...
          if (state is MarketplaceError) {
            return Scaffold(
              ...
              body: ErrorView(
                ...
                onRetry: () => context.read<MarketplaceCubit>().loadAppDetails(
                  int.parse(appId),
                ),
              ),
            );
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                if (app.fullDescription != null &&
                  ...
                  Text(l10n.appDetailsAbout, style: AppTextStyles.label),
                ...
                if (app.plans.isNotEmpty &&
                    app.plans.any((p) => p.price > 0)) ...[
                  const SizedBox(height: 28),
                  Text(
                    l10n.appDetailsAvailablePlans,
    ...
    try {
      ...
      if (!launched && context.mounted) {
        context.showSnackBar(
          AppLocalizations.of(context).appDetailsCouldNotOpenStore,
        );
      }
    } catch (_) {
      if (context.mounted) {
        ...
        children: [
          if (hasAndroid) ...[
            Expanded(
              child: _StoreButton(
                icon: Icons.android,
                label: l10n.appDetailsGooglePlay,
...
class _VideoSectionState extends State<_VideoSection> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(l10n.appDetailsPreview, style: AppTextStyles.label),
        const SizedBox(height: 8),
        ClipRRect(
          borderRadius: BorderRadius.circular(12),
          child: _initialized
...
class _PromoCodeSectionState extends State<_PromoCodeSection> {
      ...
      if (mounted) {
        setState(
          () => _error = AppLocalizations.of(
            context,
          ).appDetailsFailedLoadPromoCodes,
        );
      }
    } finally {
  ...
  Future<void> _generate() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        setState(
          () => _error =
              msg ?? AppLocalizations.of(context).appDetailsFailedGeneratePromo,
        );
      }
    } finally {
  ...
  void _copy(String code) {
    ...
    context.showSnackBar(AppLocalizations.of(context).commonCopied);
  }
                ...
                builder: (context, verState) {
                  ...
                  return GestureDetector(
                    onTap: _generating
                        ...
                        ? _generate
                        : () => context.showSnackBar(
                            l10n.appDetailsPromoUnavailable,
                            isError: true,
                          ),
                    child: Opacity(
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(
                          ...
                          tooltip: l10n.appDetailsCopyTooltip,
...
class _AccountUsageSheetState extends State<_AccountUsageSheet> {
  ...
  Future<void> _load() async {
    try {
      ...
      final filtered = all
          .where((r) => r.promoCode == widget.code.code)
          .toList();
      if (mounted) setState(() => _redemptions = filtered);
    } catch (_) {
      if (mounted) {
        setState(
          () => _error = AppLocalizations.of(context).appDetailsFailedLoadUsage,
        );
      }
    } finally {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(l10n.appDetailsCreatePromoTitle, style: AppTextStyles.heading3),
          const SizedBox(height: 4),
          Text(
            l10n.appDetailsCreatePromoSubtitle,
            style: AppTextStyles.caption.copyWith(
              color: AppColors.textSecondary,
            ),
          ),
          ...
          TextFormField(
            ...
            decoration: InputDecoration(hintText: l10n.appDetailsPromoNameHint),
          ),
          const SizedBox(height: 24),
          AppButton(
            label: l10n.appDetailsGenerate,
            onPressed: () =>
                Navigator.pop(context, (name: _nameCtrl.text.trim())),
          ),
        ],
      ),
    );
